...
public class ExplosionForce : MonoBehaviour {
	...
	private IEnumerator waitAndExplode(){
		...
		{
            if (coll.gameObject.CompareTag("Explodable"))
                coll.gameObject.GetComponent<Explodable>().explode();
		}

        Collider2D[] fragmentColliders = Physics2D.OverlapCircleAll(transform.position, radius);


        foreach (Collider2D coll in fragmentColliders)
        {
